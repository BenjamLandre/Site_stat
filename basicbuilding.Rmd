...
Ces objets, appeles vecteurs, regroupent un ensemble de donnees de même nature.   
...
a #a est inchange est comprend toujours l'ensemble des valeurs de depart
```
...
R inclut des fonctions dites de base.  
...
```{r, eval= F}
data(cars)
Modele_lineaire <- lm(formula = speed ~ dist, #a renseigner obligatoirement
                     data= cars, #idem
...
Pour chaques package vous disposez du manuel d’utilisation qui compile l’ensemble des fonctions disponibles dans le package, avec parfois un tutoriel.
...
Comme tout nouveau langage que l'on apprend, on debute generalement avec un vocabulaire (ici les fonctions) relativement pauvre.   Certaines fonctions seront decrites ici mais l’ecrasante majorite reste a decouvrir.     Plus le vocabulaire s’enrichit plus on est capable de realiser des taches specifiques, precises ou combinees a d’autres.   
