...
Dans cette bdd on remplacera certaines donnees par des valeurs manquantes pour apprendre a les reperer et les gerer efficacemment.
...
On peut chercher a connaitre l'effectif, la tendance centrale (moyenne, mediane, mode) ainsi que la dispersion (ecart-type / variance, l'etendue des donnees, ses quantiles).

La fonction summarise(), permet d'obtenir la majorite de ces indicateurs.    
D'autres fonctions permettent de les obtenir plus selectivement, un a un. Dans ce cas la, il faudra preciser la facon de gerer les donnees manquantes. Par defaut R stoppe le calcul et renvoie une erreur presence de valeurs manquantes.

**Dans le cas d'une variable qualitative, on remarquera et on gardera bien en tete que par defaut les donnees manquantes ne sont pas signalees.**
...
#indique les effectifs
table(ais_NA$sport, useNA = "always") #useNA: permet d’avoir le nb de valeurs manquantes dans une colonne supplémentaire.

#indique les pourcentages
prop.table(table(ais_NA$sport, useNA = "always"))
```
...
Parmi les fonctions de bases la fonction plot() permet de produire divers graphiques selon les arguments specifies.   
...
De maniere general, on distingue:     
- les fonctions graphiques de haut niveau, qui permettent d'obtenir l'essentiel des graphiques.   
- les fonctions graphiques de bas niveau, qui permettent d'ajouter secondairement des elements a un graphique deja existant.   
    
Les fonctions disponibles sont nombreuses, les principales sont resumees [ICI](Synthese_Graphiques_R.pdf).
Le plus simple est de les tester en faisant varier les parametres et en en deduisant leur utilisation.
...
legend("topright", #ou souhaite t'on faire apparaitre la legende
       ...
       col=unique(ais_NA$sex) #quelles sont les couleurs definies
)
}
```
...
[Synthese des graphiques](Synthese_Graphiques_R.pdf)
